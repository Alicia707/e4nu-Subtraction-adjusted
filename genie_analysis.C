	...
	for(Long64_t jentry = 0; jentry < runEntries; jentry++)
	{
       //	for (Long64_t jentry=0; jentry<Nentries;jentry++) {
		...
		if(num_p_det == 2)
		{
			...
			for(int i = 0; i < 2; i++)
			{
				V4_2prot_unsmeared[i].SetPxPyPzE(pxf[index_p[i]], pyf[index_p[i]], pzf[index_p[i]], TMath::Sqrt(m_prot*m_prot+pf[index_p[i]]*pf[index_p[i]]));
			}
			...
			if (num_pi_det == 1)
			{
			  C2p1piALL++;
				...
				TLorentzVector V4_1pi_unsmeared(pxf[index_pi[0]], pyf[index_pi[0]], pzf[index_pi[0]], Ef[index_pi[0]]);
				...
				rotation->prot2_pi1_rot_func(V4_2prot_unsmeared, V4_el_unsmeared, V4_1pi_unsmeared, V3_q, charge_pi[0], E_tot_2p,p_perp_tot_2p, P_2p1pito1p1pi);
		...
		if(num_p_det == 3)
		{
			...
			if (num_pi_det == 1)
			{
				...
				rotation->prot3_pi1_rot_func(V4_p_uncorr, V4_el, V4_pi_corr, V3_q, charge_pi[0], Ecal_3p1pi, p_miss_perp_3p1pi, P_tot_3p, V3_prot_corr, V3_pi_corr, V4_p_corr);
		...
		if(num_p_det == 1)
		{
			...
			if(num_pi_det == 3)
			{
				...
				rotation->prot1_pi3_rot_func(V3_prot_uncorr, V3_3pi_corr, V4_prot_corr, V4_3pi_corr, charge_pi, V4_el, Ecal1p3pi, p_perp1p3pi, P_1p3pi, 0);
		 		//weight_pions is 1 for CLAS data
