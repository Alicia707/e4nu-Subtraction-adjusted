...
public :
   ...
   genie_analysis(std::string, std::string, int, int , int, TTree *tree=0);
   ...
   void prot3_rot_func(std::string beam_en, TVector3 V3q, TVector3 V3prot[3],TVector3  V3prot_uncorr[3],TLorentzVector V4el,double Ecal_3pto2p[][2],double  pmiss_p_3pto2p[][2],double  P3pto2p[][2],double N_p1[3],double Ecal_3pto1p[3],double  pmiss_perp_3pto1p[3], double *N_p3det);
...
genie_analysis::genie_analysis(std::string a_target,std::string a_beam_en, int number_rotations, int choice, int percentEvents, TTree *tree) : fChain(0)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
    ...
    chain->Add(Form("/w/hallb-scshelf2102/clas/clase2/amand/eresmaid_%s_%s_hA2018_LFG_FSI_NoRadCorr_3M.root", ftarget.c_str(), fbeam_en.c_str()));
	}
