...
//		rotation->PrintQVector();
		...
		if(num_p == 2) {
			...
			if (num_pi_phot==1) {
			  C2p1piALL++;
				...
				for(int z=0; z < N_2prot; z++){ //looping over two protons
					//genie_truth analysis
					if(ProtonCounter == 2 && ChargedPionCounter == 1 && PiPlusCounter == 1)
					{
						double E_cal = Ef[ProtonIndexCounter[z]] + El + Ef[piplIndexCounter[0]]-m_prot;
						h1_E_cal_1p1pi_pipl_2pNotDetected->Fill(E_cal, histoweight);
					}
					else if(ProtonCounter == 2 && ChargedPionCounter == 1 && PiMinusCounter == 1)
					{
						double E_cal = Ef[ProtonIndexCounter[z]] + El + Ef[pimiIndexCounter[0]]-m_prot;
			...
			//	rotation->prot2_pi2_rot_func(V3_2prot_corr,V3_2prot_uncorr,V3_2pi_corr,V4_2prot_corr, V4_2pi_corr, charge_pi,V4_el, Ecal2p2pi, p_miss_perp2p2pi, P_tot_2p_2step, selection[2]);
				...
				for(int z = 0; z < N_2prot; z++){ //looping over two protons
					for(int j=0; j<2;j++){
						if(ProtonCounter == 2 && ChargedPionCounter == 2 && PiPlusCounter ==2)
						{
							double E_cal = Ef[ProtonIndexCounter[z]] + El + Ef[piplIndexCounter[z]]-m_prot;
							h1_E_cal_1p1pi_pimi_2p2piNotDetected->Fill(E_cal, histoweight);
						}
						else if(ProtonCounter == 2 && ChargedPionCounter == 2 && PiMinusCounter ==2)
						{
							double E_cal = Ef[ProtonIndexCounter[z]] + El + Ef[pimiIndexCounter[z]]-m_prot;
							h1_E_cal_1p1pi_pimi_2p2piNotDetected->Fill(E_cal, histoweight);
						}
		...
		if( num_p == 1) {
			...
			//	double histoweight = weight_pions * p_acc_ratio * e_acc_ratio * wght/Mott_cross_sec;
				...
				for(int z = 0; z < N_2pi; z++){  //to consider 2 diff. 1pi states
					//genie_truth analysis
					//genie_truth analysis
					if(ProtonCounter == 1 && ChargedPionCounter == 2 && PiPlusCounter == 2)
					{
						double E_cal = Ef[ProtonIndexCounter[0]] + El + Ef[piplIndexCounter[z]]-m_prot;
						h1_E_cal_1p1pi_pipl_2piNotDetected->Fill(E_cal, histoweight);
					}
					else if(ProtonCounter == 1 && ChargedPionCounter == 2 && PiMinusCounter == 2)
					{
						double E_cal = Ef[ProtonIndexCounter[0]] + El + Ef[pimiIndexCounter[z]]-m_prot;
						h1_E_cal_1p1pi_pimi_2piNotDetected->Fill(E_cal, histoweight);
					}
					if(charge_pi[z]>0)
					{
